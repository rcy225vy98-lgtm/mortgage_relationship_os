...
const app = express()
const port = process.env.MESSAGE_API_PORT || 8787
...
app.post('/api/generate-suggested-message', async (req, res) => {
  try {
    if (!process.env.OPENAI_API_KEY) {
      return res.status(500).json({
        error: 'Missing OPENAI_API_KEY. Add it to your .env file before using API-generated messages.',
      })
    }
...
app.post('/api/generate-suggested-messages', async (req, res) => {
  req.url = '/api/generate-suggested-message'
  return app._router.handle(req, res)
})

app.listen(port, () => {
  console.log(`Suggested message API running at http://localhost:${port}`)
})
